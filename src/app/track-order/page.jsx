          ...
          <div className="bg-white w-full max-w-md rounded-3xl p-8 text-center shadow-2xl transform transition-all">
            ...
            {paymentStatus === 'idle' && (
              <div className="text-left bg-slate-50 p-6 rounded-2xl border border-slate-200 mb-6 space-y-4">
                <p className="text-sm font-medium text-slate-600 text-center">Please transfer exactly <strong className="text-lg text-brand-600">₦{orderToPay.total.toLocaleString()}</strong> to the account below:</p>
                ...
                <button 
                  onClick={() => {
                    ...
                    setTimeout(() => {
                      ...
                      setTimeout(async () => {
                        const { data, error } = await supabase.from('orders').update({ status: 'Processing' }).eq('id', orderToPay.id).select();
                        if (!error && data) {
                          setOrders(prev => prev.map(o => o.id === orderToPay.id ? data[0] : o));
                        }
                ...
                <button onClick={() => {
                    ...
                    setPaymentStatus('idle');
                    setOrderToPay(null);
                }} className="w-full py-2 text-slate-500 font-semibold hover:text-slate-800 transition-colors">Cancel</button>
              </div>
            )}
            ...
            {paymentStatus === 'success' && (
              <div className="flex flex-col items-center gap-4 py-8">
                ...
                <p className="text-xl font-bold text-slate-800 mt-2">Payment Confirmed!</p>
